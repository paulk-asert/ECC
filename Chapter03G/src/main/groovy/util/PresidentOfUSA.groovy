...
enum PresidentOfUSA {
    ...
    public static final List<PresidentOfUSA> ALL = values().toList()


    private static final Map<Generation, List<PresidentOfUSA>> BORN_IN =
            ALL.groupBy(PresidentOfUSA::getBirthGeneration)

    static List<PresidentOfUSA> bornIn(Generation generation) {
    ...
    Set<Generation> getGenerationsServed() {
        years.collect(Generation::find).toSet()
    }
}